...
import sky.demo.basic.GameState;
...
public abstract class Panel extends Group {
	...
	public Panel(){
		...
		Engine.$camera.position.set(Config.TARGET_WIDTH/2, Config.TARGET_HEIGHT/2, 0);
		Engine.$stage.clear();
		removeBackground();
		setPosition(0, 0);
	...
	private void showPanel(){
		...
		if (fpsLabel != null && Config.fpsVisible){
			...
			Engine.$stage.addActor(fpsLabel);
		}
		if (logPane != null && Config.loggerVisible){
			logPane.setPosition(0, 0);
			Engine.$stage.addActor(logPane);
		}
	}
	...
	private void removeBackground() {
		if(bgSet){
			Engine.$stage.getRoot().removeActor(imgbg);
			bgSet = false;
		}
