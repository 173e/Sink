...
public class Archive
{
	...
	{	try
		{
			lock.acquire(400);	// Lock the archive. The lock is released
		...
		{	
			...
			else
			{
				...
				{	
					...
					InputStream in
						= new FileInputStream(stream.temporary_file());
	...
	{	private static final String STORED_ZIP		="A.stored.zip"	   ;
		private static final String COMPRESSED_ZIP	="A.compressed.zip";
		private static final String FILE_1			="root.txt"		   ;
		private static final String FILE_2			="/subdir/file.txt";
		...
		static public void main(String[] args)
		{
			...
			try
			{
				// Remove previous test files to make sure that
				// everything works as expected.

				new File( STORED_ZIP 	 ).delete();
				new File( COMPRESSED_ZIP ).delete();

				// Test to see if we can do simple reads and writes.

				Archive stored	= new Archive( 
								STORED_ZIP, Archive.UNCOMPRESSED  );

				Archive compressed	= new Archive(
								COMPRESSED_ZIP, Archive.COMPRESSED  );

				OutputStream out = 
						stored.output_stream_for( FILE_1, overwrite);
				out.write('a');
				out.write('b');
				out.close();

				out = stored.output_stream_for( FILE_2, overwrite);
				out.write('c');
				out.close();


				out = compressed.output_stream_for( FILE_1, overwrite);
				out.write('d');
				out.close();

				out = compressed.output_stream_for( FILE_2, overwrite);
				out.write('e');
				out.close();
				...
				stored 		= new Archive(
									STORED_ZIP, Archive.UNCOMPRESSED);
				compressed	= new Archive(
									COMPRESSED_ZIP,Archive.COMPRESSED );

				InputStream in = stored.input_stream_for( FILE_1 );
				t.check( "Archive.1.0", 'a', in.read() );
				t.check( "Archive.1.1", 'b', in.read() );
				t.check( "Archive.1.2", -1,  in.read() );
				in.close();

				in = stored.input_stream_for( FILE_2 );
				t.check( "Archive.2.0", 'c', in.read() );
				t.check( "Archive.2.1", -1,  in.read() );
				in.close();

				in = compressed.input_stream_for( FILE_1 );
				t.check( "Archive.3.0", 'd', in.read() );
				t.check( "Archive.3.1", -1,  in.read() );
				in.close();

				in = compressed.input_stream_for( FILE_2 );
				t.check( "Archive.4.0", 'e', in.read() );
				t.check( "Archive.4.1", -1,  in.read() );
				in.close();

				stored.close();
				compressed.close();

				// Test to see if we can do append to existing files
				...
				out = stored.output_stream_for( FILE_1, append);
				out.write('B');
				out.close();

				out = stored.output_stream_for( FILE_2, append);
				out.write('C');
				out.close();


				out = compressed.output_stream_for( FILE_1, append);
				out.write('D');
				out.close();

				out = compressed.output_stream_for( FILE_2, append);
				out.write('E');
				out.close();
				...
				in = stored.input_stream_for( FILE_1 );
				t.check( "Archive.5.0", 'a', in.read() );
				t.check( "Archive.5.1", 'b', in.read() );
				t.check( "Archive.5.2", 'B', in.read() );
				t.check( "Archive.5.3", -1,  in.read() );
				in.close();

				in = stored.input_stream_for( FILE_2 );
				t.check( "Archive.6.0", 'c', in.read() );
				t.check( "Archive.6.1", 'C', in.read() );
				t.check( "Archive.6.2", -1,  in.read() );
				in.close();

				in = compressed.input_stream_for( FILE_1 );
				t.check( "Archive.7.0", 'd', in.read() );
				t.check( "Archive.7.1", 'D', in.read() );
				t.check( "Archive.7.2", -1,  in.read() );
				in.close();

				in = compressed.input_stream_for( FILE_2 );
				t.check( "Archive.8.0", 'e', in.read() );
				t.check( "Archive.8.1", 'E', in.read() );
				t.check( "Archive.8.2", -1,  in.read() );
				in.close();

				stored.close();
				compressed.close();

				// Test to see if we can modify an existing file. Also,
				// use a large file this time to see if it's really
				// compressing correctly.

				t.println("Checking large read/write "
										+"[takes a few seconds]");
				...
				out  = stored.output_stream_for( FILE_1 );
				out2 = compressed.output_stream_for( FILE_1 );

				in = new FileInputStream("Archive.java");
				byte[] buffer = new byte[1024];
				for( int got=0; (got = in.read(buffer)) > 0 ;)
				{	out.write(buffer,0,got);
					out2.write(buffer,0,got);
				}
				out.close();
				out2.close();
				...
				in  			= stored.input_stream_for( FILE_1 );
				InputStream in2	= compressed.input_stream_for( FILE_1 );
				InputStream sample= new FileInputStream("Archive.java");
				int			c;

				t.verbose(Tester.OFF);
				while( (c = sample.read()) >= 0 )
				{	t.check( "Archive.9.0", c, in.read()  );
					t.check( "Archive.9.0", c, in2.read() );
				}
				in.close();
				in2.close();
				sample.close();
				t.verbose(Tester.RESTORE);
				t.check( "Archive.9.1", t.errors_were_found()==false,
													"Overwrite-test" );

				stored.close();
				compressed.close();

				// Test file removal. First remove one file

				stored = new Archive( STORED_ZIP );
				stored.remove( FILE_1 );
				stored.close();

				stored = new Archive( STORED_ZIP );
				try
				{
					stored.input_stream_for( FILE_1 ); // should fail
					t.check( "Archive.10.0.a", false,
											"Removal (failure a)" );
				}
				catch( ZipException e )
				{	if( e.getMessage().indexOf(FILE_1) >= 0 )
						t.check( "Archive.10.0", true, "Removal" );
					else
						t.check( "Archive.10.0", false,
											"Removal (failure b)" );
				}

				// Now remove second file, should cause an exception on
				// the close since an empty archive isn't permitted.

				stored = new Archive( STORED_ZIP );
				stored.remove( FILE_2 );
				try
				{	stored.close();
					t.check( "Archive.11.0", false, "Remove all files");
				}
				catch( ZipException e )
				{	t.check( "Archive.11.0", true, "Remove all files" );
				}

				// Finally, test the revert method.

				compressed = new Archive( STORED_ZIP );
				out = compressed.output_stream_for( "foo" );
				out.write('x');
				out.close();
				compressed.revert();
				...
				try
				{
					in = stored.input_stream_for("foo"); // should fail
					t.check( "Archive.12.0.a", false,
											"Revert (failure a)" );
				}
				catch( ZipException e )
				{	
					if( e.getMessage().indexOf("foo") >= 0 )
						t.check( "Archive.12.0", true, "Revert" );
					else
						t.check( "Archive.12.0", false,
											"Revert (failure b)" );
				}
			}
			catch( Exception e )
