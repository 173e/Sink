...
{	
	...
	{	static public void main(String[] args)
		{
			// Note that the temporary-file creation stuff is tested
			// in the FastBufferedOutputStream class's test method,
			// so you should run that test too.

			Tester	t = new Tester( args.length > 0, Std.out() );
			try
			{	
				File f = File.createTempFile("DelayedOutputStream",".test");
				OutputStream out = new DelayedOutputStream(f);
				...
				for( char c = 'a'; c <= 'z'; ++c )
					t.check("DelayedOutputStream.1", c,  in.read() );
				t.verbose(Tester.RESTORE);
				t.check("DelayedOutputStream.1", !t.errors_were_found(), "Read/Write test" );

				t.check("DelayedOutputStream.2", -1, in.read() );
				in.close();

				if( !t.errors_were_found() )
					f.delete();
				else
					Std.out().println("Test file not deleted: f.getPath()" );
			}
			catch( Exception e )
			{	t.println("DelayedOutputStream.3:  Exception Toss");
				e.printStackTrace();
			}
