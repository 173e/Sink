...
public class ParticlePanel extends JPanel {
	...
	public ParticlePanel() {
		...
		SinkStudio.log("Creating Particle Canvas");
		canvas = new  LwjglAWTCanvas(particleRenderer, false);
		initializeComponents();
		setVisible(true);
	}
	
	public void destroyCanvas(){
		SinkStudio.log("Destroying Particle Canvas");
		if(canvas != null){
			canvas.stop();
			canvas = null;
			//Gdx.app.exit();
	...
	class ParticleRenderer implements ApplicationListener, InputProcessor {
		...
		private void loadImage (ParticleEmitter emitter) {
			...
			try {
				...
				EventQueue.invokeLater(new Runnable() {
					public void run () {
						JOptionPane.showMessageDialog(ParticlePanel.this, "Error loading image:\n" + imagePath);
					}
				});
		...
		public boolean touchUp (int x, int y, int pointer, int button) {
			...
			ParticlePanel.this.requestFocusInWindow();
			return false;
		}
